...
if ( !$http_auth && ( empty($post['form']) || empty($post['CSRF_token']) ) ){
  HTP\Header::unauthorized (false,'', Generic::authErrors ($auth,"Request [Auth-App]: Authetication failed.",'self',true));
}
$rqp = [
...
if (!\in_array($app->request_timeout,["5S", "15S", "30S"])) $app->request_timeout = "15S";
$app->live = true;
if (!$app->update()) {
  $do_errors = [];
  $app->mergeErrors();
